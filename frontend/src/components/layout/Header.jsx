...
export default function Header({ onReset, hoverBar }) {
  const C             = useThemeStore((s) => s.colors);
  const { theme, toggleTheme } = useThemeStore();
  const { bars, cursor, fileName } = useSimStore();
  ...
  const V = { height: 18, width: 1, background: C.border, margin: "0 4px" };
  ...
  return (
    <div style={{ height: 46, background: C.surf, borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", padding: "0 16px", gap: 10, flexShrink: 0, fontFamily: FONT, userSelect: "none" }}>
      ...
      <div style={{ marginLeft: "auto", display: "flex", gap: 14, alignItems: "center", fontSize: 10, flexShrink: 0 }}>
        ...
        <div style={V} />
        <span style={{ color: C.dim, fontSize: 9 }}>Space=Play · →/← Step</span>
        <div style={V} />
        {/* Theme toggle */}
        <button
          onClick={toggleTheme}

...
function JournalView({ metricsFilters, bars, C }) {
  ...
  const columns = [
    ...
    { key: 'macroRegime', label: 'MACRO REGIME', width: 160, format: (v) => Array.isArray(v) ? v.join(', ') : String(v || '—') },
    { key: 'strategyType', label: 'STRATEGY TYPE', width: 160, format: (v) => Array.isArray(v) ? v.join(', ') : String(v || '—') },
    { key: 'analysisTf', label: 'ANALYSIS TF', width: 100 },
    { key: 'entryTf', label: 'ENTRY TF', width: 100 },
    { key: 'confluences', label: 'CONFLUENCES', width: 220, format: (v) => Array.isArray(v) ? v.join(', ') : String(v || '—') },
    { key: 'confluenceScore', label: 'CONFLUENCE SCORE', width: 120, format: (v) => String(v || '—') },
